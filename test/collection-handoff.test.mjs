...
test('selection, source receipts and one coalesced outbox operation survive reopening together', async t => {
  const f = await fixture(t);
  const receipt = await f.delivery.collectSelection(f.selection);
  assert.equal(receipt.status, 'recorded'); assert.equal(receipt.operationIds.length, 1);
  ...
  assert.equal(operation.collectionSources.length, 2);
  assert(operation.collectionEvidence.every(item => !('quote' in item) && /^[a-f0-9]{64}$/.test(item.quoteDigest)));
  assert.deepEqual(new Set(JSON.parse(operation.payload).facts), new Set(f.selection.facts.map(fact => fact.text)));
  ...
  for (let i = 0; i < 4; i++) await f.recreate().advance(operation.id);
...
test('concurrent handoffs persist only one result and remote session', async t => {
  const f = await fixture(t);
  const results = await Promise.all(Array.from({ length: 12 }, () => f.recreate().collectSelection(f.selection)));
  assert.equal(new Set(results.map(result => result.operationIds[0])).size, 1);
...
test('a replay after pause returns the old blocked receipt without restoring its payload', async t => {
  ...
  assert.equal(state.operations[receipt.operationIds[0]].phase, 'blocked_by_pause');
  assert.equal(state.operations[receipt.operationIds[0]].payload, undefined);
...
for (const crash of ['before-commit', 'after-commit']) {
  test(`SIGKILL ${crash} leaves either the entire old state or the entire durable handoff`, async t => {
    ...
    assert.equal(Object.keys(state.operations).length, committed ? 1 : 0);
...
test('multiple facts in one source share one receipt, and repeated facts in distinct sources share one payload item', async t => {
  ...
  assert.equal(JSON.parse(state.operations[receipt.operationIds[0]].payload).facts.length, 2);
  ...
  assert.deepEqual(JSON.parse(duplicateState.operations[duplicateReceipt.operationIds[0]].payload).facts, ['I use metric units.']);
});
